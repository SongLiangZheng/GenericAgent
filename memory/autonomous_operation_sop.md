...
- **进入执行后的第一件事**：调用code_run设置 done hook，并把“已设置”视为执行前检查项；若本轮尚未设置，**不得继续实验/写报告/收尾**
- done hook脚本：`handler._done_hooks.append("重读自主任务sop，检查你刚刚的收尾工作是否正确，不正确则改正")`，`inline_eval=True`（秘密参数）
- ≤30回合，小步快跑，边探测边实验
